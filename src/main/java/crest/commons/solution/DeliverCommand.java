...
public class DeliverCommand extends Command {
  ...
  public String asString() {
    return super.getDrone().getId() + " " + super.getCommandName() + " " + getOrder().getId() + " "
        + getProductType().getId() + " " + getQuantity();
  }

  @Override
  public int getTurns() {
    double normalizedDistance = getNormalizedDistance(this.getDrone(), this.order);
  ...
  public boolean apply(IGame game) {
    if (getTurns() == super.turnNo) {
      ...
      delivered.put(getProductType().getId(),
          delivered.get(getProductType().getId()) + getQuantity());
